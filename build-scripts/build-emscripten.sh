...
# Emscripten runtime must include its LZ4 decompressor support.
CXX_PKG_LZ4="-sLZ4=1"

make -j$(nproc)   NATIVE=emscripten   BACKTRACE=0   TILES=1   SOUND=0   TESTS=0   RUNTESTS=0   RELEASE=1   CCACHE="$CCACHE"   LINTJSON=0   PKG_CONFIG=pkg-config   CXXFLAGS="$CXX_PKG_CFLAGS"   LDFLAGS="$CXX_PKG_LIBS" $CXX_PKG_LZ4   cataclysm-tiles.js
